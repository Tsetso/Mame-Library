...
- (IBAction) tableViewDoubleClick:(id)sender
{
    ...
    if ([sender isKindOfClass:[MLGamesTableView class]])
    {
...
//        NSAlert *alert = [[[NSAlert alloc] init] autorelease];
//        [alert addButtonWithTitle:@"OK"];
//        [alert setMessageText:@"No path for selected game"];
//        [alert setInformativeText:@"Try it again after reloading the game database"];
//        [alert setAlertStyle:NSWarningAlertStyle];
...
//    }
    ...
    BOOL switchRes = [[NSUserDefaults standardUserDefaults] boolForKey:@"MLLaunchSwitchRes"];
    BOOL multiThread = [[NSUserDefaults standardUserDefaults] boolForKey:@"MLLaunchMultiThread"];
    BOOL scale = [[NSUserDefaults standardUserDefaults] boolForKey:@"MLLaunchScale"];
    NSInteger scaleFactor = [[NSUserDefaults standardUserDefaults] integerForKey:@"MLLaunchScaleFactor"];
    ...
    if (romsPath)
    {
        ...
        NSString *gamePath = [NSString stringWithFormat:@"\"%@\"",romsPath];
        [args addObject:gamePath];
        [args addObject:[game valueForKey:@"name"]];
    ...
    {
        [args addObject:@"-switchres"];
    }
    
    
    if(scale)
    {
        [args addObject:@"-prescale"];
        [args addObject:[NSString stringWithFormat:@"%ld",(long)scaleFactor]];
//        NSString *scaleParam = [NSString stringWithFormat:@"-prescale %ld",(long)scaleFactor];
//        [args addObject:scaleParam];
    }
    
    if(multiThread)
    {
        [args addObject:@"-multithreading"];
    }
